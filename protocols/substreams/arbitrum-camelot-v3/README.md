...
| Module | Output |
| --- | --- |
| `map_protocol_components` | One `camelot_v3_pool` component per `Pool` event with tokens `[token0, token1]`, contracts `[pool, operator, factory]` and static attributes `data_storage_operator` and `manual_updates` |
...
| `map_protocol_changes` | `BlockChanges`: components, balances, code and storage of pools, operators and the factory, an `update_marker` for every pool whose own or operator storage changed, and the `active_incentive` state attribute read from the pool's storage slot 4 |

All modules start at the factory creation block `101163738` so the factory is witnessed.

Pools carry `manual_updates`, so a pool is refreshed only when a transaction marks it. Factory
storage changes do not mark pools: the vault address only decides where community fees go, and
the default community fee, base fee configuration and farming address only affect pools created
or configured later. Without this, every `createPool` would refresh every Camelot pool.

The only ABI needed is the factory's, for the `Pool` event; everything else is read as storage.
